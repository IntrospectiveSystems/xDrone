class Servo {
	...
	Setup(com, fun) {
		log.i('Servo::Setup 50Hz on GPIO${this.Par.Pin}');
		const Gpio = require('gpio');

		this.Vlt.Servo = new Gpio(this.Par.Pin, { mode: Gpio.OUTPUT });

		this.Vlt.Width = this.Par.CENTER_PULSEWIDTH;
		fun(null, com);
	}

	Start(com, fun) {
		if (com.Width < this.Par.MIN_PULSEWIDTH || com.Width > this.Par.MAX_PULSEWIDTH) {
			return fun(`Servo::Start error GPIO:${this.Par.Pin}, Width ${com.Width} out of Range`, com);
		}
